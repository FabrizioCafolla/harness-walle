#!/usr/bin/env bash
# Scenario: commerce.mode "off": no cart script/styles, no /products route, and no
# Shopify Storefront request, in a fresh consumer sandbox. A fresh init's app.json
...
scenario_commerce_off() {
  ...
  # No products route at all (injected only for catalog/shop). The demo's own

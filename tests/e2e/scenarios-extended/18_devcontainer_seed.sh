#!/usr/bin/env bash
# Scenario: devcontainer sync is a mix of MANAGED marker injection and plain SEED (write-once).
# Only `setup-devcontainer.project.sh` gets a walle marker block ([walle:START]/[walle:END], `sh`
# inject kind in walle.yml). `docker-compose.project.yml` is a `seed:` entry (write-once if
# absent, never modified again, no marker), same as `.husky/` and `justfile.project`. Covers
# injection into a pre-existing file, injection into a target that doesn't exist yet (reduced
# form plus warning), idempotence on update, and `add devcontainer`.
...
scenario_devcontainer_managed_sync() {
  ...
  compose_before=$(cat "$dir/.devcontainer/docker-compose.project.yml")

  cli add devcontainer --source "$REPO_ROOT" -p "$dir" >/dev/null \
    || fail "cli add devcontainer failed" || return 1

  # setup-devcontainer.project.sh: walle's block was appended, pre-existing content preserved.
  ...
  # docker-compose.project.yml: SEED, already present before `add` ran, byte-identical after,
  # no marker of any kind.
  local compose_after
  compose_after=$(cat "$dir/.devcontainer/docker-compose.project.yml")
  [ "$compose_before" = "$compose_after" ] || fail "SEED docker-compose.project.yml must not be modified when already present" || return 1
  assert_file_contains "$dir/.devcontainer/docker-compose.project.yml" "GH_CLI_ENABLE=true" || return 1
  command -v yq >/dev/null 2>&1 && { yq eval '.' "$dir/.devcontainer/docker-compose.project.yml" >/dev/null || fail "compose file is not valid YAML" || return 1; }
  ...
  # docker-compose.project.yml stays untouched by update too (SEED, write-once).
  compose_after=$(cat "$dir/.devcontainer/docker-compose.project.yml")
  [ "$compose_before" = "$compose_after" ] || fail "update must not modify SEED docker-compose.project.yml" || return 1
  ...
  # Default init: `establish_harness_coding_base` runs first and always creates .devcontainer/
  # before walle's own inject/seed step, so no "reduced form" warning is expected here. That
  # path is only reachable from `add devcontainer` on a project that opted out at init (dir_readd
  # below).
  ...
  # This is the actual reduced-form path: no .devcontainer/ at all yet (init skipped it), `add`
  # never calls establish_harness_coding_base, so inject_marker_block creates the file with just
  # the marker block (no crash) and prints a warning about the missing base scaffold.
  ...
  cli add devcontainer --source "$REPO_ROOT" -p "$dir_readd" >"$dir_readd.add.log" 2>&1 \
    || fail "cli add devcontainer failed" || return 1
  ...
  grep -qi "no .devcontainer" "$dir_readd.add.log" || fail "expected a warning about missing .devcontainer/ (reduced form)" || return 1

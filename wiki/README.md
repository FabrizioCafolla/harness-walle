...
| Scenario                              | Approach                                                                                                  |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Start a new website project           | `cli.sh init` — scaffolds from `template/`, syncs the declared modules                                    |
...
1. A consumer runs `cli.sh init` with a project name and a list of modules. Walle seeds the `template/` scaffold (write-if-absent — safe even if run against a directory that already has files) and then syncs each module's MANAGED paths from the source (fetched from a release tag, or a local clone with `--source`).
